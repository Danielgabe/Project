...
public class TransferSurveyor extends Activity implements OnClickListener{

	private Button mConfirm;
	private Spinner spinnerInisialId;
	private TextView nourutTransferSVY;
	private Button buttonLayananSVYTransferSVY ;
	private LinearLayout linearLayoutUmumTransferSVY;
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		setContentView(R.layout.summary_transfer_surveyor);
		
		connectionDetector = new ConnectionDetector(TransferSurveyor.this);
		...
		String serviceName = globalVariable.getServiceGroupName();
		
		spinnerInisialId = (Spinner)findViewById(R.id.initialIdTransferSVY);
		nourutTransferSVY = (TextView)findViewById(R.id.nourutTransferSVY);
		nourutTransferSVY.setText(ticketNumber);
		
		linearLayoutUmumTransferSVY = (LinearLayout) findViewById(R.id.linearLayoutUmumTransferSVY);
		
		
		buttonLayananSVYTransferSVY = (Button) findViewById(R.id.buttonLayananSVYTransferSVY);
		buttonLayananSVYTransferSVY.setText(serviceName);
		if(serviceType!=null){
			...
			for(int index=0 ; index<serviceType.size() ; index++){
				...
				linearLayoutUmumTransferSVY.addView(buttonLayanan, layoutParams);
				
			}
		}
		
		mConfirm=(Button)findViewById(R.id.buttonConfirmTransferSVY);
		...
		{
			showAlertDialog(TransferSurveyor.this, "Informasi", "No Connection", false);
		}
	}
	...
	public void onClick(View view) {
		final GlobalClass globalVariable = (GlobalClass) getApplicationContext();
		switch (view.getId()) {
		case R.id.buttonConfirmTransferSVY:
			isInternetPresent = connectionDetector.isConnectingToInternet();
			...
			if(isInternetPresent)
			{
				...
				new UpdateServiceGroupTask(this).execute(globalVariable.getselmoFronLinerURL()+"/Services/SelmoFrontLiner.ashx?method=updateServiceGroup&userName="+user+"&ticketID="+ticketID+"&serviceGroupID="+serviceGroupID+"&serviceGroupName="+serviceGroupName,"updateServiceGroup");
			}
			else
			{
				showAlertDialog(TransferSurveyor.this, "Informasi", "No Connection", false);
